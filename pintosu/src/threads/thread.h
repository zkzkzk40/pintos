...
struct child
  {
    tid_t tid;                           /* tid of the thread thread_child初始为原有的tid */
    bool isrun;                          /* whether the child's thread is run successfully */
    struct list_elem child_elem;         /* list of children */
    struct semaphore sema;               /* semaphore to control waiting 存储thread中sema*/
    int store_exit;                      /* the exit status of child thread 存储thread中st_exit */
  };
...
struct thread
  {
    ...
    struct child * thread_child;        /* Store the child of this thread 当前线程的子线程*/
