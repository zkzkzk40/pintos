...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();//原子性操作begin
  // 当前线程不是空闲的线程就调用list_push_back
  // 把当前线程的元素扔到就绪队列里面，
  ...
  schedule ();
  intr_set_level (old_level);//原子性操作end
}
...
/*检查被阻塞的进程 Check the blocked thread */
void
...
thread_update_priority (struct thread *t)
{
  ...
  if (!list_empty (&t->locks))
  {
    ...
    max_priority=(lock_priority > max_priority?lock_priority:max_priority);
  }
...
running_thread (void) 
{
  ...
  asm ("mov %%esp, %0" : "=g" (esp));
  // pg_round_down， 对PGMASK取反的结果就是一个页面大小全部为0的这么个数，
  // 然后和传过来的指针做与操作的结果就是清0指针的靠右12位。
  // 一个页面12位， 而struct thread是在一个页面的最开始的，
  // 所以对任何一个页面的指针做pg_round_down的结果就是
  // 返回到这个页面最开始线程结构体的位置
  // 其实就是返回了这个页面线程的最开始指针
...
thread_schedule_tail (struct thread *prev)
{
  ...
  /* If the thread we switched from is dying, destroy its struct
     ...
     palloc().) */
  // 如果我们切换的线程状态是THREAD_DYING（代表欲要销毁的线程）的话，
  // 调用palloc_free_page
