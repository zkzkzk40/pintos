...
intr_get_level (void) 
{
  ...
  // 这个函数一样是调用了汇编指令，
  // 把标志寄存器的东西放到处理器棧上，
  // 然后把值pop到flags（代表标志寄存器IF位）上，
  // 通过判断flags来返回当前终端状态(intr_level)。
...
{
  //如果之前是允许中断的（INTR_ON）则enable否则就disable.
...
intr_context (void) 
{
    //这里直接返回了是否外中断的标志in_external_intr
